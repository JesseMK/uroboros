...
ori_path = '~/spec_2006/SPEC_CPU2006v1.0/benchspec/CPU2006/{0}/exe/{1}.ori'
rename_exe = 'mv {0} {0}.ori'
...
for l in lines:
    ...
    print '--------------', file_name, '------------------------'
    sys.stdout.flush()
    os.system(rename_exe.format(exe_path.format(
        exe_folder[file_name[:-18]], file_name)))
    print run_uroboros.format(exe_path.format(exe_folder[file_name[:-18]], file_name), ori_path.format(exe_folder[file_name[:-18]], file_name), option)
    os.system(run_uroboros.format(exe_path.format(exe_folder[file_name[
              :-18]], file_name), ori_path.format(exe_folder[file_name[:-18]], file_name), option))
    print '-----------------------------------------------------'
    sys.stdout.flush()
